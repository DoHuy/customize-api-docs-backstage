...
export const CatalogTable = (props: CatalogTableProps) => {
  ...
  const reduceEntities = entities.reduce((result: Entity[], item: Entity) => {
    if (item.spec?.owner === "huydv") {
  ...
  }, [filters.kind?.value, reduceEntities]);
  ...
  const rows = reduceEntities.sort(refCompare).map(entity => {
  ...
  return (
    <Table<CatalogTableRow>
      ...
      title={`${titlePreamble} (${reduceEntities.length})`}
      data={rows}
